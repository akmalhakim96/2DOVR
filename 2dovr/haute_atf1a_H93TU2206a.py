...
# Automatic Take off (atf) ver 1a. 2019 8/31
...
import modules.tanh2_H93TU2206a as tanh2 # 機体水平安定化のためのRobotクラス
import modules.sock4robot4b as sk # socket通信のための関数群

color_track='n' # y/n
keep_height='y' # y/n
auto_takeoff='n' # Initial this should be 'n'

GAIN_FILE='gains_atf1a_H93TU2206a.dat'
BASE_HOST ="rock7w"
ROBOT_HOST ="rp217w"
CAMERA_HOST = "rp214w" # another raspi3 on the robot
LIDAR_HOST = "rp214w"
CAMERA_PASS = "maple"
OUT_PERIOD = 0.1000

LIDAR_PYTHON = "lidar_udp_2a.py"
CAMERA_PYTHON = "ctrack6b.py"
...
# Automatic Takeoff (2019 8/31)
def Auto_Takeoff(throttle,dist,lidar_smem,rate):
   keep=50  # cm
   gain_k = -0.0001 # 
   #gain_s = 0.040
   #delta=0.2
   #past=Nrange-int(rate*delta)-1
   if dist<0.8*keep or dist > 1.2*keep:
      try:
         throttle=throttle+gain_k*(dist-keep)
...
# Automatic Landing (2019 9/3)
def Auto_Landing(throttle,dist,lidar_smem,rate):
   keep=50  # cm
   gain_k = -0.003 # 
   descent_speed=-5
   delta=0.2
   past=Nrange-int(rate*delta)-1
   descent=dist-lidar_smem[past]
   if descent>descent_speed:
      try:
         throttle=throttle+gain_k
      except:
         print("%d is out of smem range" % past)
...
def Fish_Eye(px,py):
   r0=100.0 # radius of a virtual sphere 
   #origin
   px0=160
   py0=160
   px=px-px0
...
if __name__ == '__main__':
   ...
   lidar=[0]
   cmd = "sshpass -p maple ssh pi@%s ./Lidar/%s &" % (LIDAR_HOST,LIDAR_PYTHON)
   ...
   while(ch!='q'):
      ...
      try: # キーボードでスロットルコントロール
         ...
         if(ch=='t'): # Automatic Takeoff
            print('Automaic takeoff start.')
            auto_takeoff='t'
         if(ch=='a'): # Automatic Landing
            print('Automaic Landing start.')
            auto_takeoff='a'
         ...
         # カメラ録画プロセス終了
         movie.terminate()
         cmd = "sshpass -p maple ssh pi@%s pkill picamera2.py" % CAMERA_HOST
         pkill = Popen( cmd .strip().split(" ") )
         ...
         # Lidarプロセス終了
         cmd = "sshpass -p maple ssh pi@%s pkill %s" % (LIDAR_HOST,LIDAR_PYTHON)
         pkill = Popen( cmd .strip().split(" ") )
      ...
      if now-start>OUT_PERIOD :
         ...
         try:
            #phix,phiy=Fish_Eye(c1x,c1y)
            sys.stdout.write('\033[07m'+"\r %5.1f %4d %3d : (%3d,%3d),(%3d,%3d,%3d): %3d %3d:" % (now-flight_start_time, rate, throttle, c1x,c1y,c2x,c2y,show_width,dist, l_rate))
      ...
      # LidarでAuto Takeoff
      if auto_takeoff=='t':
         throttle=Auto_Takeoff(throttle,dist,lidar_smem,l_rate)
         motor[0]=throttle
         motor[1]=throttle
         motor[2]=throttle
         motor[3]=throttle
      ...
      # Auto Landing
      if auto_takeoff=='a':
         throttle=Auto_Landing(throttle,dist,lidar_smem,l_rate)
         motor[0]=throttle
         motor[1]=throttle
         motor[2]=throttle
         motor[3]=throttle
   ...
   if os.path.exists(VIDEO_FILE):
      cmd = "sshpass -p maple ssh pi@%s ./h264.sh" % CAMERA_HOST
   ...
   pkill = Popen( cmd .strip().split(" ") )
   print("ctrack6b.py is killed")
   cmd = "sshpass -p %s ssh pi@%s pkill ctrack6b" % (CAMERA_PASS,CAMERA_HOST)
   pkill = Popen( cmd .strip().split(" ") )
   print("ctrack6b.py@%s is killed" % CAMERA_HOST)
